...
int main ()
{
    ...
    {        
        ...
        {	
            ...
            // hp = gethostbyname(ipaddress);  /*根据程序参数包含的主机名查地址，返回指针指向hostent结构*/
            // if (hp==0) 
            // {   /*当没有此主机名时*/ 
            ...
            printf("创建文件fromClient.txt用于接收客户端发来的信息......\n\n\n");
            if((outfile=open("fromClient.txt",O_WRONLY|O_CREAT|O_TRUNC,PERM))==-1) //新建文件，用来接收客户端
            {
                printf("创建文件出错\n");
                exit(1);
            } 
